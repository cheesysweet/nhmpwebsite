...
router.route("").get((req, res) => {
  carSchema.find(function(err, item) {
      if (err) {
...
router.route("/:id").delete(function(req, res) {
  ...
  carSchema.findOne({ _id : itemId }, function(err, mvi) {
      if (err) {
      res.status(404).send(err);
      }
      if (mvi !== null) { 
        carSchema.findOneAndDelete({ _id: itemId }, function(err, item) {
          if (err) {
          res.status(404).send(err)
          }
          res.status(200).json(item)
...
router.route("").post(function(req, res) {
  var item = new carSchema();
  var body = req.body;


  carSchema.findOne({ Name: body.Name, Type: body.Type }, async function(err, storedItem) { // finds out if the item exists in the database
          if (err) {
          res.send(err);
